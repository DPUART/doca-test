...
--preshared_key=swordfish
        ...
        --side=SIDE                           Side of the connection, can be r (meaning reciever, used when
...
# Tunnel type, port key and dst_port are passed as arguments to the funtion (in that order).
...
# Configure the BlueField such that IPsec packet offload can be used if 1 was passed as an argument
# To do this we need to set the steering mode to dmfs (smfs doesn't support IPsec packet offload), and also set
# the hw-offload configuration of the openvswitch to true
# If 0 was passed as an argument, then we will revert the configurations
function config_offload() {
        ...
        if [[ $1 -eq 1 ]] ; then
                echo "Setting steering mode to dmfs"
                # To set steering mode to dmfs we must first move to legacy mode and then revert to switchdev
                /opt/mellanox/iproute2/sbin/devlink dev eswitch set pci/0000:03:00.${port_num} mode legacy
                echo 'dmfs' > /sys/bus/pci/devices/0000:03:00.${port_num}/net/*/compat/devlink/steering_mode
                /opt/mellanox/iproute2/sbin/devlink dev eswitch set pci/0000:03:00.${port_num} mode switchdev
                # Add HW offload = true to the OVS configuration
                ovs-vsctl set Open_vSwitch . other_config:hw-offload=true
        elif [[ $1 -eq 0 ]] ; then
                echo "Setting steering mode to smfs"
                # To set steering mode to smfs we must first move to legacy mode and then revert to switchdev
                /opt/mellanox/iproute2/sbin/devlink dev eswitch set pci/0000:03:00.${port_num} mode legacy
                echo 'smfs' > /sys/bus/pci/devices/0000:03:00.${port_num}/net/p${port_num}/compat/devlink/steering_mode
                /opt/mellanox/iproute2/sbin/devlink dev eswitch set pci/0000:03:00.${port_num} mode switchdev
...
function revert_ipsec() {
        ...
        if [[ "$port_num" == 0 || "$port_num" == 1 ]]; then
                # Check if a connetion was established on this port number
                if [[ -d ${back_up_folder}/p${port_num} ]]; then
...
#       Checking the IPsec SAD entires if they are correct for this connection.
...
# This function recieves as parameters the devices and IP address of the PF required for the IPsec connection and checks if they are avaiable.
...
function parse_json_file() {
        # Check if file exists as absoulte path or relative path
