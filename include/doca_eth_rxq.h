...
enum doca_eth_rxq_type {
	DOCA_ETH_RXQ_TYPE_CYCLIC = 0,		/**< This mode is optimized for max packet rate. In this mode the library
						  ...
						  *  DOCA ETH RXQ context. The reccomended size of this mmap should be
						  *  calculated using doca_eth_rxq_estimate_packet_buffer_size.
						  */
	DOCA_ETH_RXQ_TYPE_MANAGED_MEMPOOL,	/**< In this mode the library will manage the memory and use various HW
						  ...
						  *  reccomended size of this mmap should be calculated using
						  *  doca_eth_rxq_estimate_packet_buffer_size.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_get_max_burst_size_supported(const struct doca_devinfo *devinfo, uint32_t *max_burst_size);
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_get_max_recv_buf_list_len_supported(const struct doca_devinfo *devinfo,
...
/**
 * @brief Get the reccomended size for the mmap buffer of a doca_eth_rxq.
 *
 * @details This function should be used for calculating the reccomended size of the doca_mmap given to
 *	    doca_eth_rxq_set_pkt_buffer().
 *
 * @note Function is irrelevant in the case of context of type DOCA_ETH_RXQ_TYPE_REGULAR.
 *
 * @param [in] type
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_eth_rxq_estimate_packet_buffer_size(enum doca_eth_rxq_type type, uint32_t rate, uint16_t pkt_max_time,
						      uint16_t max_packet_size, uint32_t max_burst_size,
						      uint8_t log_max_lro_pkt_sz, uint32_t *buffer_sz);
