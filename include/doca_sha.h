 ...
 * @param [in] devinfo
 * The DOCA device information.
 * @parma [out] max_buffer_size
 * The maximum supported buffer size in bytes.
 *
 * @return
 * DOCA_SUCCESS - The query executed successfully, *max_buffer_size holds a valid value
 * Upon failure *max_buffer_size may be uninitialized. Typical error codes for this function include:
 *  - DOCA_ERROR_INVALID_VALUE - Invalid argument(s).
 *  - DOCA_ERROR_NOT_SUPPORTED - Device has no doca_sha support.
 * Any other error reported likely means the query failed to execute.
 */
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_cap_get_max_src_buffer_size(struct doca_devinfo const *devinfo, uint64_t *max_buffer_size);
 ...
 * @param [in] devinfo
 * The DOCA device information.
 * @parma [out] min_buffer_size
 * The minimum required output buffer space.
 *
 * @return
 * DOCA_SUCCESS - The query executed successfully, *max_buffer_size holds a valid value
 * Upon failure *min_buffer_size may be uninitialized. Typical error codes for this function include:
 *  - DOCA_ERROR_INVALID_VALUE - Invalid argument(s).
 *  - DOCA_ERROR_NOT_SUPPORTED - Device has no doca_sha support.
 * Any other error reported likely means the query failed to execute.
 */
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_cap_get_min_dst_buffer_size(struct doca_devinfo const *devinfo, enum doca_sha_algorithm task_type,
...
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_task_hash_alloc_init(struct doca_sha *sha, enum doca_sha_algorithm algorithm, struct doca_buf const *src_buff,
			      struct doca_buf *dst_buff, union doca_data user_data, struct doca_sha_task_hash **task);
...
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_task_hash_set_src(struct doca_sha_task_hash *task, struct doca_buf const *src_buff);
...
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_task_hash_set_dst(struct doca_sha_task_hash *task, struct doca_buf *dst_buff);
 ...
 * by making a call to:
 *     doca_sha_task_partial_hash_set_is_final_buffer.
...
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_task_partial_hash_set_src(struct doca_sha_task_partial_hash *task, struct doca_buf const *src_buff);
...
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_task_partial_hash_set_dst(struct doca_sha_task_partial_hash *task, struct doca_buf *dst_buff);
...
DOCA_EXPERIMENTAL
doca_error_t
doca_sha_task_partial_hash_set_is_final_buffer(struct doca_sha_task_partial_hash *task);
