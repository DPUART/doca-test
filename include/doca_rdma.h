 ...
 * - DOCA_ERROR_NOT_SUPPORTED - if rdma is set on dpa datapath and is not configuered to IB mode.
 * @note stopping and restarting an RDMA context require calling doca_rdma_export() & doca_rdma_connect() again.
 */
DOCA_EXPERIMENTAL
doca_error_t
...
DOCA_EXPERIMENTAL
doca_error_t
doca_rdma_set_grh_enabled(struct doca_rdma *rdma, bool grh_enabled);
...
DOCA_EXPERIMENTAL
doca_error_t
doca_rdma_get_grh_enabled(const struct doca_rdma *rdma, bool *grh_enabled);
...
DOCA_EXPERIMENTAL
uint32_t
doca_rdma_task_receive_get_result_length(const struct doca_rdma_task_receive *task);
 ...
 * DOCA_RDMA_OPCODE_RECV_SEND_WITH_IMM or DOCA_RDMA_OPCODE_RECV_WRITE_WITH_IMM (retrievd using
...
DOCA_EXPERIMENTAL
uint32_t
doca_rdma_task_read_get_result_length(const struct doca_rdma_task_read *task);
...
DOCA_EXPERIMENTAL
uint32_t
doca_rdma_task_remote_net_sync_event_get_get_result_length(const struct doca_rdma_task_remote_net_sync_event_get *task);
